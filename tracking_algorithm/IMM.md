...
在 $K-1$ 时刻，每个模型都有一个最优估计 ${\hat x_{i,k-1}}$ , 以及协方差估计 ${\hat P_{i, k-1}}$ (表示第 $i$ 个模型，在 $k-1$ 时刻的最优估计)，再根据IMM的滤波中的正确性的概率，可以将每个模型的估计状态和协方差矩阵进行**加权融合**, 最终得到目标在 $k-1$ 时刻的最优状态以及协方差矩阵：
